...
#[async_trait]
impl FingerprintEntryBase for ComputedStyleEntry {
    ...
    async fn write_entry(&self, task: &mut TurnstileTaskEntryContext, map: &mut Map<String, Value>) -> Result<usize, Error> {
        map.insert(self.hash_key.clone(), task.fingerprint.computed_style_hash.clone().into());
        Ok(rng().random_range(4..=12))
    }
}
